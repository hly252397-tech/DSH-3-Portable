(function () {
  ...
  var current = 'deep-sea';

  function parse(id) { return (id && THEMES[id]) ? id : 'deep-sea'; }
  function readStored() {
    try { var v = localStorage.getItem(KEY); if (v && THEMES[v]) return v; } catch (e) { /* 忽略 */ }
    return null;
  }
  function effective() { return THEMES[current] ? current : 'deep-sea'; }

  function paint() {
    var theme = THEMES[effective()] || THEMES['deep-sea'];
  ...
  window.addEventListener('storage', function (event) {
    if (event.key === KEY) { current = parse(event.newValue || 'deep-sea'); paint(); }
  });
  ...
  try {
    new MutationObserver(function () {
      ...
      if (schemeChanged || presetChanged) {
        ...
        if (presetChanged) current = parse(nextPreset || 'deep-sea');
        paint();
  ...
  current = parse(queryPreset || readStored() || 'deep-sea');
  paint();
})();
