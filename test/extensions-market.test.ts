...
test('catalog preserves genuine categories, copy and package metadata', async () => {
...
test('malformed catalogs and unsafe external links are rejected', async () => {
...
test('search combines all words and category and group filters; sorting does not mutate catalog', async () => {
...
test('catalog reads use same-origin GET without caching or mutations', async () => {
...
test('HTTP failures and aborted requests never return a fake successful catalog', async () => {
...
test('extension surface has distinct content and does not install or reload on entry', async () => {
  const source = await readFile(join(process.cwd(), 'Data/DSH/profiles/web/local/dsh-sidebar-spaces/lib/client.src.js'), 'utf8')
