...
test('month labels share the 52 week columns, including 13 month fragments and leap years', async () => {
...
test('homepage cards share a responsive content column without a separate composer spacer', async () => {
  const api = await model()
...
test('sidebar search uses localized real text in its existing actionable button', async () => {
...
test('Qoder geometry is shared across color schemes and respects the resized sidebar', async () => {
...
test('coding mode copy is localized real text without replacing the native brand elsewhere', async () => {
  const source = await readFile(join(process.cwd(), 'Data/DSH/profiles/web/local/dsh-codex-ui/lib/client.js'), 'utf8')
